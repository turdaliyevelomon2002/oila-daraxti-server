...
async function checkTodayBirthdays() {
  try {
    ...
    for (const member of result.rows) {
      ...
      // DB notificationni faqat bir marta yaratamiz
      ...
      // Telegram esa har safar telegram_users bo'lsa yuboriladi
      if (tgUsers.rows.length > 0) {
    ...
    if (result.rows.length > 0) {
      console.log(`${result.rows.length} ta tug'ilgan kun tekshirildi`);
    }
  } catch (err) {
